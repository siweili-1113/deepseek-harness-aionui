...
- **Portable UI remains modest** — TUI renders plain-text goal status and generic Ralph cards. Rich ACP can carry generic text, reasoning, tool, and todo updates, but there is no continuous goal-status widget, reconnectable command output, modal goal editor, or command plane in ACP, the headless CLI, or JSON-RPC.
