...
> Committed 投影仍然仅面向自动化。[ACP rich 输出](../feature/2026-08-14-acp-rich-output-projection.md)新增一项显式交互式投影，用于传输已记录的文本、推理、工具和待办事件，但不会恢复已删除的编辑器界面。
...
`@deepseek-ai/dsh-acp` 是位于 [`packages/acp/acp`](../../../../packages/acp/acp/README.md) 下、独立于 `ui` 包组的传输层。其默认 committed 投影特意保持精简：版本协商、全新文本会话（每个会话最多允许一个进行中的提示词）、已提交的助手文本更新、按会话取消、并发会话，以及由连接负责的资源清理。提示词只承载规范要求的基线内容——文本，加上被展平为方括号文本引用的资源链接；桥接层会拒绝附加目录、MCP 服务器、超出基线的提示词内容（图片、音频、内嵌资源）、空提示词、未知会话和重叠提示词。

Committed 投影只发出已提交的 `assistant/message` 文本。显式 rich 投影会把已记录的文本与推理增量、工具活动和待办快照映射为标准 ACP 更新。标题、重试标记、终端元数据、diff、位置和资源链接仍保留在持久会话日志或 UI 专用传输层中。两种投影都不提供会话加载、列出与删除、命令、模式、配置选择器、模型切换、plan 评审或面向人类的询问。
...
ACP 保留适合 agent 与自动化的精简默认约定，而 TUI 和 Web 拥有完整的面向人类交互与展示。与已删除的编辑器桥接层相比，该包注入的服务、依赖、协议分支和生命周期状态更少，其可选 rich 投影也不将自身定位为通用编辑器入口。

自动化客户端收到完整的已提交文本，而不是 token 增量或结构化工具 UI。交互式客户端可以选择通用 rich 投影来获取推理、工具进度和待办；持久浏览、标题、恢复以及工具专用展示仍需要 host API 或产品 UI。
